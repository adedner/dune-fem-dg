...
namespace DuneODE
{
    ...
    : public Dune::Fem::LocalParameter< ROWSolverParameter, ROWSolverParameter >
  {
    ...
    virtual bool autoTolerance ( )  const
    {
      return Parameter::getValue< bool >( keyPrefix_ + "autotol", false );
    }
  ...
  public:
    ...
    : helmholtzOp_( helmholtzOp ),
      ...
      cflFactor_( getCFLFactor( timeProvider ) ),
      autoTol_( parameter.autoTolerance() ),
      firstCall_( true )
    {
      setup( butcherTable );
    }
    ...
    void solve ( DestinationType &U, MonitorType &monitor )
    {
      ...
      double linAbsTol = linAbsTol_;
      double linReduct = linReduction_;
      ...
      for( int s = 0; s < stages(); ++s )
      {
        ...
        if( autoTol_ )
        {
          const double tol = cflFactor_ * helmholtzOp_.spaceOperator().timeStepEstimate() / timeStepSize;
          linAbsTol = tol;
          linReduct = tol;
          if( linVerbose_ || firstCall_ )
          {
            if( Parameter::verbose() )
              std::cout << "ROW: tol = " << tol << " dt = " << cflFactor_ *
                helmholtzOp_.spaceOperator().timeStepEstimate() << std::endl;
            firstCall_ = false;
          }
        }
        ...
        if (preconditioner_)
        {
          typename NonlinearSolverType::LinearInverseOperatorType jInv( jOp, *preconditioner_, linReduct, linAbsTol, remLinearIts, linVerbose_ );
        ...
        */
        {
          typename NonlinearSolverType::LinearInverseOperatorType jInv( jOp, linReduct, linAbsTol, remLinearIts, linVerbose_ );
          jInv( rhs_, updateStage );
          monitor.linearSolverIterations_ += jInv.iterations();
        }
      }
  ...
  protected:
    ...
    double getCFLFactor( TimeProviderType& timeProvider ) const
    {
      typedef typename DestinationType::DiscreteFunctionSpaceType::GridType GridType;
      typedef Dune::Fem::GridTimeProvider< GridType > TP;
      TP* tp = dynamic_cast< TP* > (&timeProvider);
      if( tp )
        return tp->factor();
      else
        return Dune::Fem::Parameter::getValue( "fem.timeprovider.factor", static_cast<double>(1.0) );
    }
