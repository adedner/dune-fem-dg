...
public:
  ...
  double cs(const var_t &pvar) const {return std::sqrt(cs2(pvar));}
};
...
public:
  ...
  inline double vax() const   {return std::sqrt(vax2());}
  inline double va() const    {return std::sqrt(va2());}
...
public:
  ...
  const double boltz,amass,chi,cii,hmu,mu1;
  ...
  if (lphi < 1.0 - eps)
    {
      ...
      cons_t natflux(flux + diff * min(visc*std::sqrt(diff*diff),1.0));
