...
namespace Mhd {
  ...
  *******************************************************************************/
      ...
      case 0:
        {
          ...
          if (lrad <= MhdEPS)
            lrad = 0.0;
          lsqrtvf2mvs2 = std::sqrt( lrad );
          if (lsqrtvf2mvs2 > MhdEPS)
            {
              lrad = lvf2 - lvax2;
              if (lrad > MhdEPS)
                lalphaf = std::sqrt(lrad) / lsqrtvf2mvs2;
              else lalphaf = 0.0;
              lrad = lvf2 - lcs2;
              if (lrad > MhdEPS)
                lalphas = std::sqrt(lrad) / lsqrtvf2mvs2;
              else lalphas = 0.0;
            }
          else
            {
          ...
          lsqrtBy2pBz2 = std::sqrt(lBy * lBy + lBz * lBz);
      ...
      case 1:
        {
          ...
          pret[3] =   lbetay;
          pret[4] =   0.0;
          pret[5] = - lsgnBx * std::sqrt(4.0 * M_PI / ltau) * lbetaz;
          pret[6] =   lsgnBx * std::sqrt(4.0 * M_PI / ltau) * lbetay;
          pret[7] =   0.0;

          pret   *= pprim.vf() / std::sqrt(2.0);
      ...
      case 2:
        {
          ...
          pret[4] =   0.0;
          pret[5] =   lalphaf * lbetay * lcs2 / lvf * std::sqrt(4.0 * M_PI / ltau);
          pret[6] =   lalphaf * lbetaz * lcs2 / lvf * std::sqrt(4.0 * M_PI / ltau);
          pret[7] = - lalphas * pprim.gamma() * pprim.p();

          pret   *= lvf2 / std::sqrt(   lalphaf * lalphaf * lcs2 * (lvf2 + lcs2)
  ...
  void Mhd::r_mat(const prim_t &pprim, mat_t &pret) const
  {
    ...
    lmultvs  = lvf2 / std::sqrt(   lalphaf * lalphaf * lcs2 * (lvf2 + lcs2)
                              + lalphas * lalphas * lvf2 * (lvs2 + lcs2) );
    lmultvax = lvf / std::sqrt(2.0);
    lmultvf  = lvf  / std::sqrt(   lalphaf * lalphaf * (lvf2 + lcs2)
                              + lalphas * lalphas * (lvf2 + lvax2) );
    ...
    pret(4,0) =   0.0;
    pret(5,0) = - lalphas * lbetay * lvf * std::sqrt(4.0 * M_PI / ltau);
    ...
    pret(3,1) =   lbetay;
    pret(4,1) =   0.0;
    pret(5,1) = - lsgnBx * std::sqrt(4.0 * M_PI / ltau) * lbetaz;
    ...
    pret(4,2) =   0.0;
    pret(5,2) =   lalphaf * lbetay * lcs2 / lvf * std::sqrt(4.0 * M_PI / ltau);
    ...
    pret(4,5) =   0.0;
    pret(5,5) = - lalphaf * lbetay * lcs2 / lvf * std::sqrt(4.0 * M_PI / ltau);
    ...
    pret(3,6) = - lbetay;
    pret(4,6) =   0.0;
    pret(5,6) = - lsgnBx * std::sqrt(4.0 * M_PI / ltau) * lbetaz;
    pret(6,6) =   lsgnBx * std::sqrt(4.0 * M_PI / ltau) * lbetay;
    pret(7,6) =   0.0;
      ...
      case 0:
        {
          ...
          pret[3] = - lalphas * lbetaz * lvax * lsgnBx;
          pret[4] =   0.0;
          pret[5] = - lalphas * lbetay * lvf * std::sqrt(ltau / (4.0 * M_PI));
          pret[6] = - lalphas * lbetaz * lvf * std::sqrt(ltau / (4.0 * M_PI));
          pret[7] = - lalphaf * ltau;

          pret   *= 1.0  / ( std::sqrt(   lalphaf * lalphaf * (lvf2 + lcs2)
      ...
      case 1:
        {
          ...
          pret[3] =   lbetay;
          pret[4] =   0.0;
          pret[5] = - lsgnBx * lbetaz * std::sqrt(ltau / (4.0 * M_PI));
          pret[6] =   lsgnBx * lbetay * std::sqrt(ltau / (4.0 * M_PI));
          pret[7] =   0.0;

          pret   *= 1.0 / (std::sqrt(2.0) * pprim.vf());
      ...
      case 2:
        {
          ...
          pret[3] =   lalphaf * lbetaz * lcs * lsgnBx;
          pret[4] =   0.0;
          pret[5] =   lalphaf * lbetay * lcs2 / lvf * std::sqrt(ltau / (4.0 * M_PI));
          pret[6] =   lalphaf * lbetaz * lcs2 / lvf * std::sqrt(ltau / (4.0 * M_PI));
          pret[7] = - lalphas * ltau;

          pret   *= 1.0 / std::sqrt(   lalphaf * lalphaf * lcs2 * (lvf2 + lcs2)
                                  + lalphas * lalphas * lvf2 * (lvs2 + lcs2) );
  ...
  void Mhd::l_mat(const prim_t &pprim, mat_t &pret) const
  {
    ...
    lmultvs  = 1.0 / std::sqrt(   lalphaf * lalphaf * lcs2 * (lvf2 + lcs2)
                             + lalphas * lalphas * lvf2 * (lvs2 + lcs2) );
    lmultvax = 1.0 / ( std::sqrt(2.0) * lvf );
    ...
    pret(0,4) =   0.0;
    pret(0,5) = - lalphas * lbetay * lvf * std::sqrt(ltau / (4.0 * M_PI));
    pret(0,6) = - lalphas * lbetaz * lvf * std::sqrt(ltau / (4.0 * M_PI));
    pret(0,7) = - lalphaf * ltau;
    ...
    pret(1,3) =   lbetay;
    pret(1,4) =   0.0;
    pret(1,5) = - lsgnBx * lbetaz * std::sqrt(ltau / (4.0 * M_PI));
    pret(1,6) =   lsgnBx * lbetay * std::sqrt(ltau / (4.0 * M_PI));
    pret(1,7) =   0.0;
    ...
    pret(2,4) =   0.0;
    pret(2,5) =   lalphaf * lbetay * lcs2 / lvf * std::sqrt(ltau / (4.0 * M_PI));
    ...
    pret(5,4) =   0.0;
    pret(5,5) = - lalphaf * lbetay * lcs2 / lvf * std::sqrt(ltau / (4.0 * M_PI));
    pret(5,6) = - lalphaf * lbetaz * lcs2 / lvf * std::sqrt(ltau / (4.0 * M_PI));
    ...
    pret(6,3) = - lbetay;
    pret(6,4) =   0.0;
    pret(6,5) = - lsgnBx * lbetaz * std::sqrt(ltau / (4.0 * M_PI));
    ...
    pret(7,4) =   0.0;
    pret(7,5) =   lalphas * lbetay * lvf * std::sqrt(ltau / (4.0 * M_PI));
    pret(7,6) =   lalphas * lbetaz * lvf * std::sqrt(ltau / (4.0 * M_PI));
